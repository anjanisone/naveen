...
def poll(appPrefix, pipeline, database, table):
    res = executeNotificationClient(action ="poll", message = "[{'app_prefix':"+appPrefix+", 'pipeline':"+pipeline+", 'db':"+database+", 'table':"+table+"}]", app_prefix = appPrefix, pipeline = pipeline, messageType = "transformation", subject = "test subject")
    print("Done Polling")
    print(res)
